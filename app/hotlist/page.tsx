...
export default function HotListPage() {
  ...
  const loadDataFromSupabase = async () => {
    try {
      ...
      if (listsError) {
        ...
        setDataSources((prev) =>
          prev.map((source) => {
            const dbList = hotLists.find(
              (list) => list.hashid === source.hashId,
            );
            if (dbList) {
              return {
                ...source,
                status: "active" as const,
                lastUpdate: new Date(dbList.last_updated).toLocaleString(
                  "zh-CN",
                ),
              };
            }
      ...
      if (itemsError) {
        ...
        const processedItems: HotListItem[] = items.map((item) => ({
          ...
          url: item.url || "",
          popularityScore:
            Number.parseInt(item.extra?.replace(/[^\d]/g, "") || "0") ||
            Math.floor(Math.random() * 10000),
          readCount: item.extra || "0万",
          publishTime: new Date(item.collected_at),
          author: `热榜作者`,
          category:
            dataSources.find((s) => s.hashId === item.hot_list_hashid)
              ?.category || "未知",
          isCollected: false,
        }));
  ...
  const fetchSingleHotList = async (hashId: string) => {
    try {
      setDataSources((prev) =>
        prev.map((source) =>
          source.hashId === hashId ? { ...source, status: "loading" } : source,
        ),
      );
